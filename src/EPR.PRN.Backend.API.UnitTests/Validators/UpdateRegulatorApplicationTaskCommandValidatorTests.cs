...
namespace EPR.PRN.Backend.API.UnitTests.Validators
{
    ...
    public class UpdateRegulatorApplicationTaskCommandValidatorTests
    {
        ...
        {
            var model = new UpdateRegulatorApplicationTaskCommand { Status = StatusTypes.Completed };
            var result = _validator.TestValidate(model);
        ...
        {
            var model = new UpdateRegulatorApplicationTaskCommand { Status = StatusTypes.Completed, Comment = string.Empty };
            var result = _validator.TestValidate(model);
            result.ShouldNotHaveValidationErrorFor(x => x.Comment);
        }
    }
