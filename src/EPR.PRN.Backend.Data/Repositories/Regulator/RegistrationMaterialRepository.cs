...
public class RegistrationMaterialRepository(EprContext eprContext) : IRegistrationMaterialRepository
{
    ...
    public async Task SaveOverseasReprocessingSites(UpdateOverseasAddressDto overseasAddressSubmission)
    {
        ...
        var registrationMaterialId = await GetRegistrationMaterialIntegerId(overseasAddressSubmission.RegistrationMaterialId);

        await DeleteOverseasReprocessingSites(overseasAddressIds, registrationMaterialId);

        var overseasAddressesAfterDelete = await eprContext.OverseasAddress
            .Where(x => x.RegistrationMaterialId == registrationMaterialId)
            .ToListAsync();

        await UpdateOverseasReprocessingSites(overseasAddressesAfterDelete, overseasAddressSubmission.OverseasAddresses);

        var overseasAddressesAfterUpdate = await eprContext.OverseasAddress
           .Where(x => x.RegistrationMaterialId == registrationMaterialId)
           .ToListAsync();

        await CreateOverseasReprocessingSites(overseasAddressSubmission.OverseasAddresses, overseasAddressesAfterUpdate, registrationMaterialId);
    }

    private async Task DeleteOverseasReprocessingSites(List<Guid> overseasAddressIds, int registrationMaterialId)
    {
        var overseasAddresses = await eprContext.OverseasAddress
            .Where(x => x.RegistrationMaterialId == registrationMaterialId)
            .ToListAsync();

        var overseasAddressesToDelete = overseasAddresses.Where(x => x.RegistrationMaterialId == registrationMaterialId
                    && !overseasAddressIds.Contains(x.ExternalId)).Select(x => x.ExternalId).ToList();

        if (overseasAddressesToDelete.Count() > 0)
        {
            var entitiesToDelete = await eprContext.OverseasAddress
            .Where(x => overseasAddressesToDelete.Contains(x.ExternalId))
            .ToListAsync();

            eprContext.OverseasAddress.RemoveRange(entitiesToDelete);
            await eprContext.SaveChangesAsync();
        }
    }
    ...
    private async Task CreateOverseasReprocessingSites(List<OverseasAddressDto> overseasAddresses, List<OverseasAddress> overseasAddressesAfterDb, int registrationMaterialId)
    {
        var overseasAddressesToCreate = overseasAddresses
            ...
            .Select(x => new OverseasAddress
            {
                ...
                CountryId = eprContext.LookupCountries
        ...
        await eprContext.OverseasAddress.AddRangeAsync(overseasAddressesToCreate);

        await eprContext.SaveChangesAsync();
    }

    private async Task<int> GetRegistrationMaterialIntegerId(Guid registrationMaterialId)
    {
        var registrationMaterialIdInt = await eprContext.RegistrationMaterials.Where(x => x.ExternalId == registrationMaterialId).Select(x => x.Id).SingleOrDefaultAsync();

        if (registrationMaterialIdInt == 0)
            throw new KeyNotFoundException("RegistrationMaterial not found");

        return registrationMaterialIdInt;
    }
