    ...
    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        if (From == DateTime.MinValue || From == DateTime.MaxValue || From == default(DateTime))
        {
            yield return new ValidationResult(
                "From date must be a valid date and not the minimum, maximum, or default date.",
                new[] { nameof(From) }
            );
        }
        ...
        {
            yield return new ValidationResult(
                "To date must be a valid date and not the minimum, maximum, or default date.",
                new[] { nameof(To) }
            );
        }
        ...
        {
            yield return new ValidationResult(
                "The 'To' date must not be earlier than the 'From' date.",
                new[] { nameof(To) }
            );
        }
    }
