        ...
        private void CreateTaskMappings()
        {
            ...
            CreateMap<Registration, RegistrationTaskOverviewDto>()
                ...
                .ForMember(dest => dest.Materials,
                    opt => opt.MapFrom(src => src.Materials.Where(m => m.IsMaterialRegistered)))
                
                .ForMember(dest => dest.Tasks,
                    opt => opt.MapFrom((src, dest, _, context) =>
                        MapTasks(src.Tasks, src.ApplicantRegistrationTasksStatus, context)));
        }

        private static List<RegistrationTaskDto> MapTasks(List<RegulatorRegistrationTaskStatus>? registrationTasks, List<ApplicantRegistrationTaskStatus>? applicantRegistrationTasks, ResolutionContext context)
        { 
